...
class SubharmonicFieldEngine:
    """
    2D Potential Field Engine implements harmonic and subharmonic fields. - UPDATE: it's 3D now
    ...
    def solve_harmonic_field(self, verbose: bool = False) -> int:
        """
        I AM A LEGEND IF I MAKE THIS WORK :,] - I made it work
    ...
    def solve_subharmonic_field(self, a_att: float = 0.01, a_rep: float = 1.0,
        ...
        Repulsive field: phi_rep = +a * exp(-k * r^2)
            ...
            danger distance d0 = sqrt(n_dims / (2 * k))  (Eq. 20)

        Parameters:
        - a_att : float - attractive field gain (Eq. 21)
        - a_rep : float - repulsive field amplitude (Eq. 13)
        - k_rep : float - repulsive field steepness; danger distance = sqrt(n_dims / (2 * k))
        ...
        self.a_rep = a_rep
        self.k_rep = k_rep
        n_dims = len(self.goal_position)   # 2 for 2D grid, 3 for 3D analytical
        self.danger_distance = np.sqrt(n_dims / (2.0 * k_rep))
    ...
    def get_temporal_derivative_at_position(self, position: np.ndarray) -> float:
        ...
        """
        if self.subharmonic_mode:
            return self.compute_dphi_dt(position)
    ...
    def visualize(self, show_gradient: bool = True, gradient_skip: int = 5, agent_position: Optional[np.ndarray] = None,show_temporal_derivative: bool = False,
        ...
        if show_temporal_derivative:
            if self.subharmonic_mode:
                # In analytical mode the grid finite-difference is always zero unless
                # recompute_grid=True was passed to update_field_analytical.
                # Build the temporal-derivative grid analytically instead.
                dphi_dt = np.zeros_like(self.phi)
                for i in range(self.grid_resolution):
                    for j in range(self.grid_resolution):
                        grid_pos = self._grid_to_world(i, j)
                        dphi_dt[i, j] = self.compute_dphi_dt(grid_pos)
            else:
